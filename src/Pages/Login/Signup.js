...
const Signup = () => {
  ...
  const handleSubmit = (event) => {
    ...
    //======= start imgbb upload image function 
          ...
          .then(result => {
            ...
            updateUserProfile(name, imageData.data.display_url)
              .then(() => {
                ...
                // veryfiy email
                verifyEmail().then(() => { toast.success('please verify your email') });

              }).catch((error) => { console.log(error); }); //!updateProfile ==
          }).catch(err => {
            toast.error(err)
            setLoading(false)
          }) //!== create user ==
      }).catch(err => console.log(err)) //!== imgbb upload image ==
